...
COPY runtime/scripts/install_hugo.sh nd_docker_scripts/install_hugo.sh
RUN nd_docker_scripts/install_hugo.sh

COPY runtime/scripts/install_micromamba.sh nd_docker_scripts/install_micromamba.sh
RUN nd_docker_scripts/install_micromamba.sh
ENV MAMBA_ROOT_PREFIX=/opt/micromamba
ENV PATH="$HOME/.local/bin:$PATH"

COPY runtime/scripts/install_extra.sh nd_docker_scripts/install_extra.sh
RUN nd_docker_scripts/install_extra.sh

COPY runtime/scripts /nd_docker_scripts

# s6 service for static-web-server
COPY runtime/services/static-web-server /etc/services.d/static-web-server
